...
protected:
  ...
  void get_to_operate() {
    ...
    // operate
    uint8_t line[] = {1};
    k->startOperation(Machine::None, 0, NUM_NEEDLES - 1, false, line);
  }
...
TEST_F(KnitterTest, test_startOperation) {
  uint8_t line[] = {1};
  // Not in ready state
  ASSERT_EQ(k->startOperation(Machine::None, 0, NUM_NEEDLES - 1, false, line),
            false);
  ...
  ASSERT_EQ(k->startOperation(Machine::None, 1, 0, false, line), false);

  // stopNeedle equal to NUM_NEEDLES
  ASSERT_EQ(k->startOperation(Machine::None, 0, NUM_NEEDLES, false, line),
            false);

  // null pointer passed as line
  ASSERT_EQ(
      k->startOperation(Machine::None, 0, NUM_NEEDLES - 1, false, nullptr),
      false);
}
...
TEST_F(KnitterTest, test_operate) {
  ...
  k->startOperation(Machine::None, START_NEEDLE, STOP_NEEDLE, true, line);
